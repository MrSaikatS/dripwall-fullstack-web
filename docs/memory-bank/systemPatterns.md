...
2. Server: Validates user session → validates/sharp-processes image → uploads to S3 → persists URL to user DB record → cleans up S3 file on DB write failure
3. Client: On success, revokes object URL preview; no client-side `authClient.updateUser` call needed
